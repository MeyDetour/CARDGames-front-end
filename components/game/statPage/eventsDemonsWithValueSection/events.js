...
export default function statEventsDemonsWithValueSectionEventSection(gameData) {
  ...
  return ` 
         ${data
           .map((log, index) => {
             if (log.testType == "event" || log.testType == "eventFromAction"  || log.testType == "internalEvent") {
               let event = log;
               return /*html */ `
                     <details class="elementDetails ${log.testType == "eventFromAction" ? "eventFromAction" : ""}">
                        ...
                        <div class="elementContent">
                           ...
                           <div class="modificationDetail">
                              <span class="span" style="font-weight: bold;">Détails des modifications </span>
                                 ...
                                 <div class="detailWrapper">
                                    ${event?.event?.withValue
                                      .map(
                                       ...
                                       <div class="rowInWrapper"><span class="span">Ordre ${index + 1} :</span><span class="span">${gameData.roomInDb.events.events.find((event) => event.id === withValueEventObject.id)?.name || "Unknown Event"}</span></div>
                                    `,
                                      )
                                      .join("")}  
                                 </div>
